...
#SBATCH --output /research/users/justoh/slurm_logs/ostpre_cat_logs/ostpre_%A_%a.err    # Standard output goes to here
#SBATCH --error  /research/users/justoh/slurm_logs/ostpre_cat_logs/ostpre_%A_%a.out    # Standard error goes to here
#SBATCH --array=1-100%25     # Array range and number of simultanous jobs, note that these indexes refer to
...
cd ${CATDIR}
bash cat_batch_bids3.sh ${BIDSDIR}/${SUBJ}
echo  "Done" >> ${DONEDIR}/${SUBJ}_${ROWINDEX}_long.txt  # Optional: write a separate log-file to allow easy monitoring  
