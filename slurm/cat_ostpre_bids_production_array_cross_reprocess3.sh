...
#SBATCH --mem-per-cpu=15000	# Reserve 6 GB RAM for a task, this is the memory required for a single subject, 
...
#SBATCH --output /research/users/justoh/slurm_logs/ostpre_cat_logs/ostpre_re4_%A_%a.err    # Standard output goes to here
#SBATCH --error  /research/users/justoh/slurm_logs/ostpre_cat_logs/ostpre_re4_%A_%a.out    # Standard error goes to here
#SBATCH --array=1-255%20     # Array range and number of simultanous jobs, note that these indexes refer to
...
SUBJ=$(sed -n "$ROWINDEX"p ${BIDSDIR}/reprocess_cross3.txt)
...
echo  "Done" >> ${DONEDIR}/${SUBJ}_${ROWINDEX}_recross2.txt  # Optional: write a separate log-file to allow easy monitoring  
